...
class User(Base):
    ...
    balance: Mapped[int] = mapped_column(Integer, default=1_000_000)  # demo: cấp 1.000.000đ khi đăng ký
...
class PlinkoRound(Base):
    ...
    slot_index: Mapped[int] = mapped_column(Integer)        # 0..rows, which bucket the ball landed in
    path: Mapped[str] = mapped_column(String(32))           # e.g. "0101101..." (0=left,1=right) per row
