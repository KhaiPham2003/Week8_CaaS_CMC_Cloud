...
export default function Layout({ user, env = "LAB", onLogout, page = "dashboard", onNavigate, children }) {
  const navItem = (key, label) => (
    <div
      key={key}
      onClick={() => onNavigate && onNavigate(key)}
      className={
        "cursor-pointer rounded-xl px-3 py-2 font-semibold " +
        (page === key
